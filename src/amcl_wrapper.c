...
int xtt_crypto_create_ecdsap256(xtt_ed25519_pub_key *pub_key, xtt_ed25519_priv_key *priv_key) {
    ...
    octet pub = {.val = (char*)pub_key->data, .len = 0, .max = sizeof(xtt_ed25519_pub_key)};
    octet priv = {.val = (char*)priv_key->data, .len = sizeof(xtt_ed25519_priv_key), .max = sizeof(xtt_ed25519_priv_key)};
...
int xtt_crypto_sign_ecdsap256(unsigned char* signature_out,
    ...
    octet priv_sign_key_in = {.val = (char*)priv_key->data, .len = sizeof(xtt_ed25519_priv_key), .max = sizeof(xtt_ed25519_priv_key)};
...
int xtt_crypto_verify_ecdsap256(const unsigned char* signature,
    ...
    octet pub = {sizeof(xtt_ed25519_pub_key), sizeof(xtt_ed25519_pub_key), (char*)pub_key->data};
    octet msg_in = {.val = (char *)msg, .len = msg_len};
